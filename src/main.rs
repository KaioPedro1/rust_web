use bongo::{
    configuration::{get_local_configuration, Settings},
    redis_utils,
};
use env_logger::Env;
...
async fn main() -> std::io::Result<()> {
    ...
    redis_utils::set_initial_redis_state(&mut redis_connection, connection_pool.clone())
        .await
        .expect("Failed to fetch initial data");
