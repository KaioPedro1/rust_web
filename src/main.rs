...
async fn main() -> std::io::Result<()> {
    ...
    let mut redis_connection = redis::Client::open(config.redis.redis_url)
    ...
    redis_utils::set_initial_redis_state(&mut redis_connection, connection_pool.clone()).await;
    ...
    run(listener, connection_pool, redis_connection)?.await
}
